...
def __VerifyInputDict(setup_params, input_dict):
    """
    ...
    for unit_index in range(index_counter):
        ...
        if not group_key:
            raise ValueError(
                ...
                + "This unit should be registered as keyword under jobrunner/instruments"
            )
        ...
        for index in range(
            index_range[unit_index, 0] + 1, unit_range[group_index, 1] + 1
        ):
            if len(params[index]) > 4:
                if params[index][:4] == " " * 4 and params[index][4] != " ":
                    if group_key in setup_dict:
